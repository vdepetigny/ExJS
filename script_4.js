...
console.log(`Cet array contient ${journalists.length} journalistes en tout`)

// Combien d'handles contiennent un numéro ?
let withNumber = []
journalists.forEach(journalist => {
  if (journalist.match(/\d/)) {
  	withNumber.push(journalist) 
  }
});
console.log(`Cet array contient ${withNumber.length} journalistes avec des pseudos chiffre`)
...
console.log(`Cet array contient ${audeArray.length} journalistes ayant "Aude" dans leur pseudo`)

// Combien commencent par une majuscule (première lettre juste après le @) ?

let pseudoWithCapitalFirst = []
journalists.forEach(journalist => {
  if (journalist.match(/^.{1}[A-Z]/)) {
  	pseudoWithCapitalFirst.push(journalist) 
  }
});
console.log(`Cet array contient ${pseudoWithCapitalFirst.length} journalistes une majuscule juste après @`)

// Combien contiennent une majuscule ?

let pseudoWithCapital = []
journalists.forEach(journalist => {
  if (journalist.match(/[A-Z]/)) {
    pseudoWithCapital.push(journalist) 
  }
});
console.log(`Cet array contient ${pseudoWithCapital.length} journalistes contiennent une majuscule`)

// Combien y a-t-il de underscore _ dans tous les pseudos confondus ?

let pseudoWithUnderscore = []
let journal = journalists.join()
console.log(`Cet array contient ${journal.split("_").length - 1} journalistes contiennent un underscore`)

// Trie la liste de handles par ordre alphabétique.
console.log(`Voici la liste alphabétique des pseudos :`)
...
console.log(`@epenser est le ${journalists.indexOf("@epenser")}ème élément de l'array`)
